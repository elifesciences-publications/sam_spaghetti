...
def load_sequence_signal_images(sequence_name, image_dirname, verbose=False, debug=False):
    ...
    for time in xrange(max_time):
        ...
        if os.path.exists(image_dirname+"/"+filename+"/"+filename+"_signal_data.csv"):
            sequence_filenames += [filename]

    if len(sequence_filenames)>0:
        logging.info("--> Loading sequence images "+sequence_name+" : "+str([f[-3:] for f in sequence_filenames]))
        ...
        for filename in sequence_filenames:
            ...
            file_signals = [c for c in file_df.columns if (not "center" in c) and (not "layer" in c) and (not 'Unnamed' in c)]
            ...
            for signal_name in file_signals:
                ...
                if os.path.exists(signal_image_file):
                    ...
                    if os.path.exists(signal_image_file):
                        ...
                        logging.warn("  --> Unable to find : "+filename+" "+signal_name)
                logging.info("  <-- Loading : "+filename+" "+signal_name+" ["+str(current_time() - start_time)+" s]")

    return signal_images


def load_sequence_signal_data(sequence_name, image_dirname, aligned=False, verbose=False, debug=False):
    ...
    if len(sequence_filenames)>0:
        logging.info("--> Loading sequence data "+sequence_name+" : "+str([f[-3:] for f in sequence_filenames]))
        for filename in sequence_filenames:
            if aligned:
                data_filename = image_dirname+"/"+filename+"/"+filename+"_aligned_L1_normalized_signal_data.csv"
            else:
                data_filename = image_dirname+"/"+filename+"/"+filename+"_signal_data.csv"
...
def signal_image_plot(signal_images, signal_data, signal_names=None, filenames=None, aligned=False, filtering=False, projection_type="L1_slice", reference_name='TagBFP', membrane_name='PI', resolution=None, r_max=120., microscope_orientation=-1, verbose=False, debug=False):
    ...
    if len(filenames)>0:
        ...
        for filename in filenames:
            for signal_name in signal_names:
                ...
                if filtering:
                    start_time = current_time()
                    logging.info("  --> Filtering : "+filename+" "+signal_name)
                    filtered_img = gaussian_filter(filtered_img,sigma=nuclei_sigma/np.array(signal_img.voxelsize),order=0)
                    logging.info("  <-- Filtering : "+filename+" "+signal_name+" ["+str(current_time() - start_time)+" s]")
        ...
        for i_time, (time, filename) in enumerate(zip(file_times,filenames)):
            ...
            if projection_type == "L1_slice":
                start_time = current_time()
                logging.info("  --> Computing Z-map : "+filename)
                ...
                logging.info("  <-- Computing Z-map : "+filename+"   ["+str(current_time() - start_time)+" s]")

                for i_signal, signal_name in enumerate(signal_names):
                    start_time = current_time()
                    logging.info("  --> Slicing : "+filename+" "+signal_name)

                    image_slices[signal_name][filename] = filtered_signal_images[signal_name][filename][slice_coords[filename]].reshape(xx.shape)

                    logging.info("  <-- Slicing : "+filename+" "+signal_name+" ["+str(current_time() - start_time)+" s]")

            elif projection_type == "max_projection":
            ...
            for i_signal, signal_name in enumerate(signal_names):
                if not image_views[signal_name].has_key(filename):
...
def signal_nuclei_plot(signal_data, signal_names=None, filenames=None, aligned=False, reference_name='TagBFP', r_max=120., microscope_orientation=-1, verbose=False, debug=False):
    ...
    if len(filenames)>0:
        ...
        if signal_names is None:
            signal_names = [c for c in signal_data[filenames[0]].columns if (not "center" in c) and (not "layer" in c) and (not 'Unnamed' in c)]
